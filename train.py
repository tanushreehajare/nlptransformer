...
def run_epoch(
    ...
    """One epoch of training or evaluation. Returns average per-token loss."""
    ...
    print(f"[epoch {epoch_num}] {tag} loss={avg_loss:.4f}  "
          f"ppl={math.exp(min(avg_loss, 20)):.2f}  ({elapsed:.1f}s)")
    return avg_loss


# ══════════════════════════════════════════════════════════════════════
#  GREEDY DECODING  (kept for evaluate_bleu speed during training)
# ══════════════════════════════════════════════════════════════════════
...
def greedy_decode(
    ...
    """Greedy decoding — fast, used for val BLEU during training."""
    model.eval()
    src      = src.to(device)
...
def evaluate_bleu(
    ...
    """Decode the test set and return corpus BLEU (0-100).
    use_beam=True for final eval; False (greedy) for fast val-epoch checks.
    """
    model.eval()
    ...
    with torch.no_grad():
        for src, tgt in test_dataloader:
            ...
            for i in range(src.size(0)):
                ...
                if use_beam:
                    memory   = model.encode(src_i, src_mask_i)
                    pred_ids = model._beam_search(
                        memory, src_mask_i,
                        beam_size=beam_size, max_len=max_len, alpha=0.6,
                    )
                else:
...
def run_training_experiment(
    *,
    epochs:          int   = 50,
    batch_size:      int   = 128,
    d_model:         int   = 512,    # full paper model size
    N:               int   = 6,      # full paper depth
    num_heads:       int   = 8,
    d_ff:            int   = 2048,   # full paper FFN size
    dropout:         float = 0.1,
    smoothing:       float = 0.1,
    warmup_steps:    int   = 8000,   # longer warmup for bigger model
    use_noam:        bool  = True,
    fixed_lr:        float = 1e-4,
    ...
    try:
        ...
        wandb_run = wandb.init(
            ...
            config=dict(
                ...
                smoothing=smoothing, warmup_steps=warmup_steps,
                use_noam=use_noam, fixed_lr=fixed_lr,
                scale_attention=scale_attention, positional=positional,
            ),
        )
    ...
    # Build model with NO auto-download (we're training from scratch)
    model = Transformer(
        ...
        checkpoint_path="__skip__",   # sentinel: skip auto-download
        gdrive_id=None,
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Use greedy for fast val BLEU during training; beam only for final eval
        val_bleu = evaluate_bleu(model, val_loader, tgt_vocab,
                                 device=device, max_len=80, use_beam=False)
        print(f"[epoch {epoch}] val BLEU (greedy) = {val_bleu:.2f}")
